...
walkToo Expo/React Native 앱(`app/`, `components/`, `hooks/`, `styles/`) 작업 시 핵심 룰. 공통 룰은 `AGENTS.md`를 따른다.
...
- **글로벌 관점**: 현재 한국어 단일이지만 `lib/i18n`이 이미 깔려 있으므로 새 카피는 i18n 키로 추가한다.
...
- **경계는 디렉토리가 아니라 의존 방향으로 정의된다.** 화면(`app/`) → 훅(`hooks/`) → Service(`server/`) → Repository 단방향. 역참조 금지.
- **상태는 가까운 곳에 둔다.** 화면 로컬에서 충분하면 zustand store(`stores/`)로 끌어올리지 않는다. 반대로 여러 화면이 공유하면 화면 안에서 prop drilling하지 않는다.
...
- 폰트는 `NeoDunggeunmo` 하나. 직접 `Text`보다 `components/base/Text.tsx`를 우선 사용한다.
- 토큰의 source of truth는 `styles/theme.ts`, `styles/type.ts`, `styles/diaryThemes.ts`. 새 화면은 토큰을 사용한다.
- 간격은 `SPACING`, 레이아웃은 `LAYOUT` 토큰 우선. radius는 `theme.radius.lg`(8) 이하 유지, pill/rounded 과용 금지.
- 그림자는 blur 없는 **솔리드** 픽셀 스타일(`shadowRadius: 0`).
- 컴포넌트: `PixelCard`, `PixelBadge`, `Button`, `Row`/`Column`/`Box`, 아이콘은 `components/base/Icon.tsx`의 `Icon` 래퍼.

## 데이터·외부 연동

- 서버 호출은 **`server/` Service만** import. Repository 직접 호출 금지.
- 장소 검색·지도는 `lib/location/` provider 추상화를 거친다. 지도 SDK/키가 없을 때도 앱이 깨지지 않게 fallback을 유지한다.
...
- 스티커/테이프/프레임 자산은 `assets/diary/`를 재사용한다.
