# walkToo+ 월 구독 (RevenueCat) 세팅 가이드

walkToo+ 월 구독은 [RevenueCat](https://www.revenuecat.com)을 통해 Apple/Google IAP를 처리한다.
...
| 항목 | 값 |
|---|---|
| **Product ID** (Apple/Google 동일) | `com.walktoo.plus.monthly` |
...
2. **Create Subscription**
   ...
   - Reference Name: `walkToo+ Monthly`
   - Product ID: `com.walktoo.plus.monthly`
3. 가격 설정:
   - 한국 (KRW): **₩990/월**
   - 글로벌 (USD): **$0.99/month**
...
2. **구독 만들기**
   - 상품 ID: `com.walktoo.plus.monthly`
   - 이름: `walkToo+ 월 구독`
   - 설명: `walkToo+ 기능을 매월 사용할 수 있어요`
3. 가격: **₩990/월** (다른 국가는 자동 환산)
4. 상태: **활성**
...
3. **Products**:
   - **+ Product** → Apple → `com.walktoo.plus.monthly` 추가
   - **+ Product** → Google → `com.walktoo.plus.monthly` 추가
4. **Entitlements**:
...
5. **Offerings**:
   ...
   - **Packages**: `Monthly` 패키지에 위 product들 attach
   - **Mark as current** 체크
...
- 리뷰 노트에 "walkToo+ is a monthly subscription. Free trial is implemented in-app and does not use Apple's introductory offer." 명시
- 환불 정책 안내 페이지(앱 내 또는 웹) 링크 제공 권장

## 9. 향후 개선 (Phase 2)

- **RevenueCat ↔ Supabase webhook**: 갱신/해지/만료 이벤트를 받아 Supabase 권한을 정확히 동기화
- **트라이얼 만료 알림**: D-1 push notification으로 conversion 유도
- **환불 처리**: RevenueCat의 refund webhook 받아 권한 revoke RPC 호출
